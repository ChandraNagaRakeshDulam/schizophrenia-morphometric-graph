...
def main():
    ...
    # Representation/training recipe must be frozen internally.
    # Density and hidden width are read from the MIND checkpoint because
    # they were selected using ds000030 only.
    assert ckpt["representation"] == "mind"
    assert ckpt["model_kind"] == "gat"
    assert float(ckpt["density"]) in {0.10, 0.20, 0.30}
    assert int(ckpt["hidden"]) in {32, 64}
